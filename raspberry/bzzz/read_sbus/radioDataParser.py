...
# Maximum pitch value corresponding to the top position of the stick 
# (30deg = 0.52rad)
...
class RadioDataParser:
    ...
    def __map_radio_to_angle(self, 
                             x, 
                             stick_min=RADIO_STICK_MIN,
                             stick_max=RADIO_STICK_MAX):
        """
        Maps radio stick positions to corresponding angles in radians linearly.

        :param x: Stick position data.
        :param stick_min: Minimum value of radio stick, 
                          defaults to RADIO_STICK_MIN
        :param stick_max: Maximum value of radio stick, 
                          defaults to RADIO_STICK_MAX
        :return: angle in radians.
        """
        return (-PITCH_MAX_RAD + (x - stick_min) / (stick_max - stick_min) 
                * 2 * PITCH_MAX_RAD)

    def __map_trimmer_to_percentage(self, 
                                    x, 
                                    stick_min = RADIO_STICK_MIN, 
                                    stick_max = RADIO_STICK_MAX):
        """
    ...
    def pitch_reference_angle_rad(self):
        ...
        """
        return self.__map_radio_to_angle(
            self.m_channelData[RADIO_CHANNEL_PITCH])
    ...
    def roll_reference_angle_rad(self):
        ...
        """
        return self.__map_radio_to_angle(
            self.m_channelData[RADIO_CHANNEL_ROLL])
    ...
    def yaw_rate_reference_rad_sec(self):
        ...
        return (-RADIO_MAX_YAW_RATE_RAD_SEC + 2 * RADIO_MAX_YAW_RATE_RAD_SEC 
                * self.rawRatePercentage)
    ...
    def switch_D(self):
        """
        Checks the position of the switch D.
    ...
    def trimmer_VRA_percentage(self):
        ...
        """
        return self.__map_trimmer_to_percentage(
            self.m_channelData[RADIO_CHANNEL_VRA])
    ...
    def trimmer_VRC_percentage(self):
        ...
        """
        return self.__map_trimmer_to_percentage(
            self.m_channelData[RADIO_CHANNEL_VRC])
    ...
    def trimmer_VRB_percentage(self):
        ...
        """
        return self.__map_trimmer_to_percentage(
            self.m_channelData[RADIO_CHANNEL_VRB])
    ...
    def trimmer_VRE_percentage(self):
        ...
        """
        return self.__map_trimmer_to_percentage(
            self.m_channelData[RADIO_CHANNEL_VRE])
    ...
    def format_radio_data_for_sending(self):
        ...
        - `SWTCHs` is an integer whose last five bits contain the encoded 
           position data of the A, B, C, and D switches, and the encoding is 
           as follows:
            ...
            For switches `A`, `B`, and `D` (since these are only two-way
            switches), each were assigned a single bit with 0 indicating that
            the switch is in off state and 1 otherwise. For switch C, as it is
            a three-way switch, it was assigned with two bits with 00 = DOWN,
            01 = MID, and 10 = UP.    
        
        :return: Formatted string that contains the processed radio data, 
                 where each data point is separated by a comma.
        """
        reArrangedYPRTData = [self.yaw_rate_reference_rad_sec(), 
                              self.pitch_reference_angle_rad(), 
                              self.roll_reference_angle_rad(),
                              self.__map_prcnt(
                                  self.throttle_reference_percentage(),
                                  ZERO_ROTOR_SPEED, 
                                  ABSOLUTE_MAX_PWM)]
        bitEncodedSwithcesData = ((self.switch_B() << 4) | 
                                  (self.switch_A() << 3) | 
                                  (self.switch_C() << 1) | 
                                   self.switch_D())
        reArrangedABCETrimmersData = [self.trimmer_VRA_percentage(), 
        ...
        return reArrangedData + [bitEncodedSwithcesData], ",".join(map(
        lambda x: str(x)[:5], reArrangedData)) + f",{bitEncodedSwithcesData}"
