...
if __name__ == '__main__':
    ...
    feature_names = ("datetime", "z_gnss", "z_tof", "z_bar", "z_ref", "z_hat",
                     "v_hat", "alpha_0", "alpha_1", "d_bar", "d_gnss", "tau")
    logger = DataLogger(num_features=11,
    ...
    def altitude_estimator(radio_data, y, rtk_status):
        tau = radio_data.throttle_reference_percentage()
        altitude_kf.update(tau, 0, 0, y, rtk_status)
    ...
    def control_loop(tof, barometer, gnss, esp_bridge):
        ...
        y = np.array([y_gnss, y_tof, y_bar])
        if y_tof > min_altitude_hold_altitude:
            altitude_estimator(radio_data, y, rtk_status)  # deals with nans
