...
class Scheduler:
    ...
    def schedule(self, function_name: str, function_obj, function_call_frequency, function_call_count=0):
        ...
        if self.__use_threading:
            f_thread = bzzz.thread_this.run_thread_every_given_interval(interval=1./function_call_frequency, function_to_run=function_obj, num_times_to_run=function_call_count)
    ...
    def kill(self, function_name):
        ...
        if function_name in self.__scheduled_events:
            if self.__use_threading:
                ...
                self.num_times_called(function_name, change_value=True, to_value=self.function_call_count(function_name))
        else:
