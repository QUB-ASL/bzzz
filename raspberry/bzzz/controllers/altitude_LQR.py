...
class LQR:
    ...
    def __init__(self, sampling_frequency=10, initial_alpha_t=0, initial_beta_t=0):
        ...
        self.__reference_tracker_dynamics = np.zeros(
            (self.__A.shape[0] + self.__C.shape[0], self.__A.shape[1] + self.__B_t.shape[1]), dtype=np.float64)
    ...
    def control_action(self,
        ...
        if recalculate_dynamics:
            self.__recalculate_dynamics(
                alpha_t=self.__alpha_t, beta_t=self.__beta_t, pitch_rad=pitch_rad, roll_rad=roll_rad)
        ...
        throttle_command = self.__kappa@(current_states_z_and_vz -
                                         self.__x_and_u_bar[:2, :]) + self.__x_and_u_bar[2, :]
...
if __name__ == "__main__":
    ...
    print(lqr.control_action(current_states_z_and_vz=np.array(
        [[2], [1]]), alpha_t=1, beta_t=-9.81, reference_altitude_mts=1, recalculate_dynamics=True))
