...
print("alpha hat = %f"%np.max(x_hat_t[:, 2]), "\nc hat = %f"%np.max(x_hat_t[:, 3]))

fig, sub_plts = plt.subplots(2)

sub_plts[0].plot(x_hat_t[:, 0])
sub_plts[0].plot(y_t)
sub_plts[0].legend([r"$\hat{x}_t$", r"$y_t$"])

sub_plts[1].plot(x_hat_t[:, 2])
sub_plts[1].plot(x_hat_t[:, 3])
