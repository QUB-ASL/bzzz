...
#       Now Invert:
#           00111101 011
#       Above is *little-endian*, so the '1' on the right is 1024 in decimal
#       To read little-endian, reverse the bits and read left-to-right
#            110 00111101
#       In Decimal
#            1597
...
# current bit in the working packet which we are waiting for data at
_working_packet_ptr = 0
# stores result as packet comes into system
_working_packet = bau.zeros(_PACKET_LENGTH)


# stores the last packet that the system recorded
_latest_complete_packet = bau.zeros(_PACKET_LENGTH)
# stores tick at which the packet was recorded
_latest_complete_packet_timestamp = 0
...
def _on_change(gpio, level, tick):
    ...
    # 10 microseconds per data bit, so number of bits since last state change
    # is time difference/10
    num_bits = round((time_elapsed)/10)
    # enter the level *before* this state change which is the inverse of
    # current change.
    bit_val = bool(-level+1)
...
class SbusReader:
    ...
    def translate_packet(self, packet):
        ...
        # holds the bits of the 16 11-bit channel values
        channel_bits = ba.bitarray(176)
        ...
        for packet_bits_ptr in range(_UART_FRAME_LENGTH, _UART_FRAME_LENGTH
            ...
            channel_bits[channel_bits_ptr:channel_bits_ptr +
                         8] = ~packet[packet_bits_ptr+1:packet_bits_ptr+9]
            channel_bits_ptr += 8
        ...
        for channel_ptr in range(0, 16*11, 11):
            ...
            # little endian.
            ret_list.append(bau.ba2int(ba.bitarray(
                channel_bits[channel_ptr:channel_ptr+11], endian='little')))
            ...
            try:
                ...
                try:
                    ...
                    for i, val in enumerate(channelValList):
                        stdscr.addstr(int(i/2), (i % 2)*25,
                                      f'Channel # {i+1}: {val}   ')
                    stdscr.addstr(
                        8, 0, f'Packet Age(milliseconds): {self.get_latest_packet_age()}        ')
                    if (_is_connected):
