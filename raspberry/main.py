...
if __name__ == '__main__':
    ...
    def read_ToF_run_kf_and_LQR():
        ...
        # NOTE: DO NOT DIVIDE tof_dis BY 1000 here to get altitude measurements in m.
        # There are checks if tof_dis == -1 in the code for outlier detection.
        ...
        distance_from_tof_sensor = tof.distance

        if distance_from_tof_sensor == -1:
            ...
            last_valid_altitude_measurement_mts[0] = distance_from_tof_sensor/1000
        ...
        if is_drone_flying_close_to_ground[0]:
            z_hat[0] = current_altitude_snap_shot_mts[0] if distance_from_tof_sensor == - \
                1 else distance_from_tof_sensor/1000
            print_debug(
            ...
            x_est = kf.update(Tref_t[0], euler[1], euler[2],
                              np.nan if distance_from_tof_sensor == -1 else distance_from_tof_sensor/1000)
            ...
            if use_altitude_hold[0]:
                if not is_current_altitude_snap_shot_taken[0]:
                    current_altitude_snap_shot_mts[0] = distance_from_tof_sensor/1000
