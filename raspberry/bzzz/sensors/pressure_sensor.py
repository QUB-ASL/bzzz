...
def _calculate_altitude_from_pressure(pres, temp, initial_pressure):
    """
    Calculate altitude from pressure and temperature using the barometric 
    formula.
    ...
    alt = ((((temp+celsius_to_kelvin)*k)/((molecular_mass)*g))
           * math.log(initial_pressure/pres))
    return alt
...
class BMP180Sensor:
    ...
    def __init__(self,
        ...
        :param data_processor: Object to process the data readings 
                               (default: MedianFilter)
        :param window_length: Number of readings to keep in the moving window 
                              (default: 3)
        :param DEVICE_ADDRESS: I2C address of the BMP180 sensor (default: 0x77)
        :param initial_pressure: Reference pressure at sea level for altitude
                                 calculations (default: 101325 Pa)
    ...
    def __get_measurements_in_background(self):
        """
        Thread method to continuously read measurements from the sensor, 
        process them, and log them.
        """
        while self.__keep_running:
            pres, temp = self.__pressure_temp_from_sensor()

            if self.__initial_pressure > 0:
                alt = _calculate_altitude_from_pressure(
                    pres, temp, self.__initial_pressure)
            else:
                alt = 0
    ...
    def altitude(self):
        ...
        with self.__lock:
            return self.__data_processor.process(self.__values_cache[:, :],
                                                 cursor=self.__cursor)[1]

    def __pressure_temp_from_sensor(self):
        """
        Read raw pressure and temperature data from the BMP180 sensor.

        This method communicates with the BMP180 sensor over the I2C bus to
        fetch the raw temperature and pressure readings. It then uses the
        sensor's calibration data to convert these raw readings into true
        temperature (in Celsius) and pressure (in Pa) values based on the
        sensor's calibration coefficients.

        The method handles the entire process of sending the correct commands
        to the sensor, reading the raw data, and applying the necessary
        conversion formulas as outlined in the BMP180 datasheet.

        :return: A tuple containing the true pressure (in Pa) and temperature
                 (in Celsius).
        """
    ...
    def __initialise(self, num_initial_readings=30):
        ...
        :param num_initial_readings: Number of readings to average for initial 
                                     altitude.
        """
        sum_pressures = 0
        do_run = True
