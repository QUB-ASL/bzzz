...
def LSTM_NN(file_name,
    ...
    def normalize(data):
        ...
        return (data + max_wind_speed) / (2 * max_wind_speed)

    dataset_U = normalize(df_wind_U)
    dataset_V = normalize(df_wind_V)
    dataset_W = normalize(df_wind_W)
    ...
    def to_sequences(dataset, seq_size=1):
        x, y = [], []
        for i in range(len(dataset) - seq_size - prediction_horizon):
            x.append(dataset[i:i + seq_size])
            y.append(dataset[i + seq_size + prediction_horizon - 1])
    ...
    def reshape_data(data):
        x, y = to_sequences(data, seq_size)
        return x.reshape((x.shape[0], x.shape[1], 1)), y

    trainX_U, trainY_U = reshape_data(train_U)
    testX_U, testY_U = reshape_data(test_U)
    trainX_V, trainY_V = reshape_data(train_V)
    testX_V, testY_V = reshape_data(test_V)
    trainX_W, trainY_W = reshape_data(train_W)
    testX_W, testY_W = reshape_data(test_W)

    def build_lstm_model():
        model = Sequential()
        model.add(LSTM(nodes_1, input_shape=(seq_size, 1), return_sequences=(Layer_2 or Layer_3 or Layer_4)))
        if Layer_2:
            model.add(LSTM(nodes_2, return_sequences=(Layer_3 or Layer_4)))
        if Layer_3:
            model.add(LSTM(nodes_3, return_sequences=Layer_4))
        if Layer_4:
            model.add(LSTM(nodes_4))
        model.add(Dense(1))
        model.compile(loss='mean_squared_error', optimizer='adam', metrics=['mae'])
        return model

    # Train and save models for U, V, W
    for name, trainX, trainY, testX, testY in [('U', trainX_U, trainY_U, testX_U, testY_U),
                                               ('V', trainX_V, trainY_V, testX_V, testY_V),
                                               ('W', trainX_W, trainY_W, testX_W, testY_W)]:
        model = build_lstm_model()
        model.fit(trainX, trainY, validation_data=(testX, testY), epochs=no_of_epochs, verbose=0)
        model.save(f"model_{name}_LSTM.keras")

# Example call
LSTM_NN(file_name='raspberry/data/wind_data/16-09-23--18-35/16-09-23--18-35_N_10',
        input_size=10,
        prediction_horizon=40,
        no_of_epochs=12,
        Layer_2=True,
