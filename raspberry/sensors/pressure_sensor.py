...
class PressureSensor:
    def __init__(self, DEVICE_ADDRESS: int = 0x77, num_latest_readings_to_keep: int = 5, reference_pressure_at_sea_level=None):
    ...
    def __readBmp180Id(self):
        ...
        REG_ID     = 0xD0
        chip_id, chip_version = self.smbus.read_i2c_block_data(self.DEVICE_ADDRESS, REG_ID, 2)
    ...
    def __readBmp180(self):
        ...
        cal = self.smbus.read_i2c_block_data(self.DEVICE_ADDRESS, REG_CALIB, 22)
        ...
        time.sleep(0.005)
        (msb, lsb) = self.smbus.read_i2c_block_data(self.DEVICE_ADDRESS, REG_MSB, 2)
        UT = (msb << 8) + lsb

        # Read pressure
        self.smbus.write_byte_data(self.DEVICE_ADDRESS, REG_MEAS, CRV_PRES + (OVERSAMPLE << 6))
        time.sleep(0.04)
        (msb, lsb, xsb) = self.smbus.read_i2c_block_data(self.DEVICE_ADDRESS, REG_MSB, 3)
    ...
    def _calculate_altitude_from_pressure(self):
        ...
        self._current_altitude = 44330 * (1 - (self._current_pressure/self.reference_pressure_at_sea_level)**(1/5.255))
