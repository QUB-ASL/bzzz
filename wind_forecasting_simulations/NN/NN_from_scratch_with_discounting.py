...
class RBFNetworkQR:
    ...
    def fit(self, X, y):
        ...
        """
        F = np.diag([self.forgetting_factor**(len(y)-n -1) for n in range(len(y))])

        A = self._calculate_A(X)
        # Solve for the weights using QR decomposition
        self.Q, self.R = np.linalg.qr(A.T @ F @ A)
        self.b = A.T @ F @ y
        rhs = self.Q.T @ self.b
        lhs = self.R 
    ...
    def update(self, 
        ...
        """
        Update the RBF Network with a new sample and discounting (forgetting factor).
        ...
        if len(updated_centers) != self.num_centers:
            ...
            F = np.diag([self.forgetting_factor**(len(self.y)-n -1) for n in range(len(self.y))])
        ...
        self.Q, self.R = spla.qr_update(self.Q, self.R, ((self.forgetting_factor** -0.5)*new_A), ((self.forgetting_factor**-0.5) *new_A))

        # Update the b vector
        self.b = self.b + (new_y * new_A)/self.forgetting_factor
...
if __name__ == "__main__":
    ...
    forgetting_factor = 0.992 # Forgetting factor
    ...
    print(f'for window_size: {window_size} and num_centers: {num_centers} and adaptation_rate: {adaptation_rate} and forgetting_factor: {forgetting_factor}')
