...
def medianFilter(file_name,
    ...
    # Calculating mean
    res['Wind_Speed'] = df_wind.Wind_Speed.rolling(number_samples).median().round(4)
    res['Wind_Speed_2D'] = df_wind.Wind_Speed_2D.rolling(number_samples).median().round(4)
    res['H_direction'] = df_wind.H_direction.rolling(number_samples).median().round(4)
    res['V_direction'] = df_wind.V_direction.rolling(number_samples).median().round(4)
    res['U_axis'] = df_wind.U_axis.rolling(number_samples).median().round(4)
    res['V_axis'] = df_wind.V_axis.rolling(number_samples).median().round(4)
    res['W_axis'] = df_wind.W_axis.rolling(number_samples).median().round(4)
    
    #set index
    res.index = pd.date_range(res.Index_2[0], res.Index_2.iloc[-1], freq="25L")
    
    #save file
    res.to_csv(f'{file_name}_median_filtered_N_{str(number_samples)}.csv', index=False)
