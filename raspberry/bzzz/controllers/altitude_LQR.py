...
class LQR:
    ...
    The system dynamics are time-varying and the state space matrix Bt varies
    with battery drainage. This implementation of LQR uses already available
    'DARE' solver from `control` module and the solution is obtained every time
    the system dynamics are updated.
    """

    def __init__(self, sampling_frequency=10, 
                 initial_alpha_t=0, 
                 initial_beta_t=0):
        """Constructor

        :param sampling_frequency: Sampling frequency in Hz, defaults to 10
        :param initial_alpha_t: initial value of vertical acceleration 
                                parameter alpha, defaults to 0
        :param initial_beta_t: initial value of vertical acceleration 
                               parameter beta, defaults to 0
        """
        # sampling frequency
        ...
        # equilibrium points "The hover estimate"
        self.__x_and_u_bar = np.zeros((3, 1))
        self.__reference_tracker_dynamics = np.zeros(
            (self.__A.shape[0] + self.__C.shape[0], self.__A.shape[1] 
             + self.__B_t.shape[1]), dtype=np.float64)
    ...
    def set_alpha_beta(self, 
                       throttle_scaling_factor, 
                       throttle_acceleration_offset):
    ...
    def __recalculate_dynamics(self, alpha_t, beta_t, pitch_rad=0, roll_rad=0):
        """Update alpha_t, beta_t, drone pitch_rad and roll_rad, and 
        recalculate the system dynamics (A, B, C, and D matrices).
        ...
        # then, 
        # a_z_global_frame = cos(pitch) * cos(roll) * (alpha * Tref + beta)
        # => a_z_global_frame = cos(pitch) * cos(roll) * alpha * Tref 
        # + cos(pitch) * cos(roll) * beta
    ...
    def __update_x_and_u_bar(self, reference_altitude_mts):
        ...
        """
        # update the equilibrium points, this solution is analytically obtained
        # and so doesn't require finding a matrix inverse every time
    ...
    def __calculate_stabilising_gain(self):
        """Calculate the stabilising feedback gain using LQR method, call this 
        function whenever the system dynamics are updated 
        """
    ...
    def control_action(self,
        ...
        :param current_states_z_and_vz: current states of altitude in m and 
                                        velocity in m/s in a 2x1 numpy array
        :param alpha_t: vertical acceleration parameter alpha, defaults to 0
        :param beta_t: vertical acceleration parameter beta, defaults to 0
        :param reference_altitude_mts: reference altitude in mts
        :param recalculate_dynamics: True to update and recalculate system 
                                     dynamics, defaults to False
        :param pitch_rad: current drone pitch in radians, defaults to 0
        :param roll_rad: current drone roll in radians, defaults to 0
        :param k11: if not None, overwrites the stabilising gain of kappa at 
                    matrix position (1, 1) to given value, defaults to None
        :param k12:  if not None, overwrites the stabilising gain of kappa at 
                     matrix position (1, 2) to given value, defaults to None
        :return: Throttle command in percentage in a numpy 1x1x1 matrix
        """
        if recalculate_dynamics:
            self.__recalculate_dynamics(
                alpha_t=self.__alpha_t, beta_t=self.__beta_t, 
                pitch_rad=pitch_rad, roll_rad=roll_rad)
            self.__update_x_and_u_bar(
                reference_altitude_mts=reference_altitude_mts)
            if k11 == None or k12 == None:
                self.__calculate_stabilising_gain()
        self.__set_kappa_11_and_12(k11, k12)
        throttle_command = (self.__kappa@(current_states_z_and_vz 
                                          - self.__x_and_u_bar[:2, :])
                            + self.__x_and_u_bar[2, :])
        return throttle_command[0, 0]


# test script
if __name__ == "__main__":
    lqr = LQR(initial_alpha_t=1, initial_beta_t=-9.81)
    print(lqr.control_action(current_states_z_and_vz=np.array(
        [[2], [1]]), alpha_t=1, beta_t=-9.81, reference_altitude_mts=1, 
