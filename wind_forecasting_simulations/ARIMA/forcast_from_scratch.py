...
# y_10 = np.array([1.007,1.053,1.11,1.179,1.276,1.32,1.356,1.394,1.419,1.402,1.386,1.357,1.327])
...
#     error = np.delete(error, [-1, -2, -3, -4 ,5])
#     error = np.append(error,(df_wind.Wind_Speed[x + 8] - all_forecast[-5])) 

#     for z in range(5):
#         ARMA = ar1*(y[-1]) + ar2*(y[-2]) + ar3*(y[-3]) + ar4*(y[-4]) + ar5*(y[-5]) + ma1*(error[-1]) + ma2*(error[-2]) + ma3*(error[-3]) + ma4*(error[-4]) + ma5*(error[-5]) + ma6*(error[-6])+ ma7*(error[-7]) + ma8*(error[-8])
#         y = np.append(y, ARMA)
...
# df.index = pd.date_range(df_wind.Index_2[0], df_wind.Index_2[112], freq="25L")
# df['real_wind'] = df_wind.Wind_Speed.values[0:113]
